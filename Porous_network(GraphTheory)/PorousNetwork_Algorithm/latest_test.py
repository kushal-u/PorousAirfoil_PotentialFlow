...
# ==============================================================================
def generate_structured_mesh(X, Y, xc_panels, yc_panels, x_start, x_end, n_target_pores, pore_radius, mu):
    valid_indices = [i for i, xc in enumerate(xc_panels) if x_start <= xc <= x_end]
    if len(valid_indices) > n_target_pores:
        idx_to_keep = np.linspace(0, len(valid_indices)-1, n_target_pores, dtype=int)
        porous_indices = [valid_indices[i] for i in idx_to_keep]
    else:
        porous_indices = valid_indices

    boundary_nodes = np.array([[xc_panels[i], yc_panels[i]] for i in porous_indices])
    internal_points = []
    
    # Simple Spine
    for i in range(0, len(boundary_nodes), 2): 
        bx, by = boundary_nodes[i]
        internal_points.append([bx, by * 0.1])

    internal_points = np.array(internal_points)
    all_points = np.vstack((boundary_nodes, internal_points))
    ...
    for i, p in enumerate(all_points):
        ...
        if i < len(boundary_nodes):
            G.nodes[i]['type'] = 'boundary'
            G.nodes[i]['panel_idx'] = porous_indices[i]
        else:
            G.nodes[i]['type'] = 'internal'

    path = mpath.Path(np.column_stack((X, Y)))
    
    for simplex in tri.simplices:
        for u, v in [(simplex[0], simplex[1]), (simplex[1], simplex[2]), (simplex[0], simplex[2])]:
            if G.has_edge(u, v): continue
            
            # Skip skin connections
            if G.nodes[u]['type'] == 'boundary' and G.nodes[v]['type'] == 'boundary':
                continue
            ...
            if path.contains_point(mid):
                ...
                cond = (np.pi * pore_radius**4) / (8 * mu * L)
                G.add_edge(u, v, length=L, cond=cond, type='standard')

    # Rescue Orphans
    internal_indices = [n for n in G.nodes() if G.nodes[n]['type'] == 'internal']
    if len(internal_indices) > 0:
        for i in range(len(boundary_nodes)):
            if G.degree(i) == 0:
                dists = np.linalg.norm(all_points[internal_indices] - all_points[i], axis=1)
                nearest_idx = internal_indices[np.argmin(dists)]
                p_surf = all_points[i]
                p_int = all_points[nearest_idx]
                L = np.linalg.norm(p_surf - p_int)
                cond = (np.pi * pore_radius**4) / (8 * mu * L)
                G.add_edge(i, nearest_idx, length=L, cond=cond, type='rescue')

    G.remove_nodes_from(list(nx.isolates(G)))
    return G, porous_indices
...
def plot_results(aero, Cp, Cp_solid, V_leakage, CL, CL_solid, CD, CD_solid, G, output_dir="porous_airfoil_results"):
    ...
    if not os.path.exists(output_dir):
        ...
        print(f"-> Saving images to existing directory: {output_dir}")
...
def run_simulation():
    ...
    print("-> Generating Structured Internal Mesh...")
    G, porous_pids = generate_structured_mesh(X, Y, aero.XC, aero.YC, X_START, X_END, N_PORES, PORE_RADIUS, MU)
