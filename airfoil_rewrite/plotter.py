...
class Visualizer:
    ...
    def plot_all(self, aero_solid, aero_porous, porous_net,
             Cp, Cp_solid, P_nodes,
             q_solid, gamma_solid, q_porous, gamma_porous):
        print(f"-> Generating plots in {self.output_dir}...")
        self._plot_geometry_cp(aero_porous, porous_net, Cp, Cp_solid)
        self._plot_flow_field_comparison(
            aero_solid, aero_porous, porous_net, P_nodes,
            q_solid, gamma_solid, q_porous, gamma_porous
        )
        self._plot_pressure_field_comparison(
            aero_solid, aero_porous, porous_net, P_nodes,
            q_solid, gamma_solid, q_porous, gamma_porous
        )
    ...
    def _plot_flow_field_comparison(self, aero_solid, aero_porous, net, P_nodes,
        ...
        cases = [
            ("SOLID: |V| + Streamlines", aero_solid, q_solid, gamma_solid, False),
            ("POROUS: |V| + Streamlines + Network Dir", aero_porous, q_porous, gamma_porous, True),
        ]

        directed_edges = self._compute_directed_network_edges(net, P_nodes)

        cf = None
        for ax, (title, aero_case, q_case, gamma_case, overlay_net) in zip(axes, cases):
            u, v = aero_case.compute_velocity_field(xg, yg, q=q_case, gamma=gamma_case)
            mag = np.sqrt(u**2 + v**2)
    ...
    def _plot_pressure_field_comparison(self, aero_solid, aero_porous, net, P_nodes,
        ...
        cases = [
            ("SOLID: Pressure + Streamlines", aero_solid, q_solid, gamma_solid, False),
            ("POROUS: Pressure + Streamlines + Network Dir", aero_porous, q_porous, gamma_porous, True),
        ]
        directed_edges = self._compute_directed_network_edges(net, P_nodes)

        cf = None
        for ax, (title, aero_case, q_case, gamma_case, overlay_net) in zip(axes, cases):
            u, v = aero_case.compute_velocity_field(xg, yg, q_case, gamma_case)      
